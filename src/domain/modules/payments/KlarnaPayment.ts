...
export class KlarnaPayment extends BasePaymentProvider implements IPaymentProvider, IKlarnaPaymentProvider {
    ...
    async initPaymentIntent(data: any): Promise<any> {
        try {
            if (super.initSDK()) {
                const intentResult = await new SDKs.Klarna.Infra.Services.Payment().initIntent(data);
                return intentResult;
            }
            return null;
        }
    ...
    async createOneTimePaymentOrder(data: any): Promise<any> {
        try {
            if (super.initSDK()) {
                const intentResult = await new SDKs.Klarna.Infra.Services.Payment().createOneTimePaymentOrder(data);
                return intentResult;
            }
            return null;
        }
    ...
    async getOrderDetails(data: any): Promise<any> {
        try {
            if (super.initSDK()) {
                const payment = new SDKs.Klarna.Infra.Services.Payment();
