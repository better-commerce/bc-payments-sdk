...
export default class Api {
    ...
    static async call(url: string, method: string, params?: any, cookies?: any, headers?: any): Promise<any> {
        ...
        let options = { url, method, headers, cookies, baseUrl: SDKs.Checkout.Config.CheckoutEnvironment.baseUrl, };

        if (params) {
            if (method?.toUpperCase() === Domain.Enums.RequestMethod.GET) {
                options = { ...options, ...{ params: params }, };
            } else if (method?.toUpperCase() === Domain.Enums.RequestMethod.POST || method?.toUpperCase() === Domain.Enums.RequestMethod.PUT) {
