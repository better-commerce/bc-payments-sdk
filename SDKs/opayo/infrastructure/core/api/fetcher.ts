...
const SingletonFactory = (function () {
    ...
    const axiosInstance = axios.create({
        baseURL: SDKs.Opayo.Config.OpayoEnvironment.baseUrl,
    ...
    axiosInstance.interceptors.request.use(
        (config: any) => {
            ...
            const auth = Buffer.from(`${SDKs.Opayo.Config.OpayoEnvironment.getIntegrationKey()}:${SDKs.Opayo.Config.OpayoEnvironment.getIntegrationPassword()}`).toString("base64");
    ...
    function createAxiosResponseInterceptor() {
        const interceptor = axiosInstance.interceptors.response.use(
            ...
            (error: any) => {
                ...
                const extras = SDKs.Opayo.Config.OpayoEnvironment.getExtras();
                ...
                const url = new URL('api/v1/merchant-session-keys', SDKs.Opayo.Config.OpayoEnvironment.getAuthUrl());
                const auth = Buffer.from(`${SDKs.Opayo.Config.OpayoEnvironment.getIntegrationKey()}:${SDKs.Opayo.Config.OpayoEnvironment.getIntegrationPassword()}`).toString("base64");
                const requestConfig = {
                    url: url.href,
                    method: Domain.Enums.RequestMethod.POST,
                    data: { vendorName: SDKs.Opayo.Config.OpayoEnvironment.getVendorName() },
...
const fetcher = async ({ url = '', method = 'post', data = {}, params = {}, headers = {}, cookies = {}, baseUrl = "", }: any) => {
    const computedUrl = new URL(url, baseUrl || SDKs.Opayo.Config.OpayoEnvironment.getBaseUrl());
    const config: any = {
        method: method,
