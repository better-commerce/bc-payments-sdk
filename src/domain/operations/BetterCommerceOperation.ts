...
import { ICommerceProvider } from "../contracts/ICommerceProvider";
import { PaymentStatus } from "../enums/PaymentStatus";
import { PaymentMethodType } from "../enums/PaymentMethodType";
import { Checkout as CheckoutGateway, Klarna as KlarnaGateway, PayPal as PayPalGateway, Stripe as StripeGateway, ClearPay as ClearPayGateway } from "../enums/PaymentStatus";
...
export class BetterCommerceOperation implements ICommerceProvider {
    ...
    async processPaymentHook(data: IPaymentHookProcessingData): Promise<any> {
        try {
            ...
            const paymentTransactionStatus = Utils.PaymentUtil.getPaymentTransactionStatus(paymentMethodTypeId, hookData);
            ...
            if (paymentTransactionStatus.toLowerCase() !== PaymentTransactionStatus.NONE) {
                ...
                if (paymentMethodTypeId === PaymentMethodTypeId.PAYPAL) {
                    const details: string = await Utils.PaymentUtil.getPaymentTransactionOrderId(paymentMethodTypeId, hookData);
                    ...
                    orderId = await Utils.PaymentUtil.getPaymentTransactionOrderId(paymentMethodTypeId, hookData);
                }
                ...
                if (orderId != Defaults.Guid.Value) {
                    ...
                    if (orderResult?.id && orderResult?.id != Defaults.Guid.Value) {
                        ...
                        if (payments?.length) {
                            ...
                            if (paymentMethodTypeId !== PaymentMethodTypeId.PAYPAL) {
                                paymentNo = Utils.PaymentUtil.getPaymentNo(paymentMethodTypeId, paymentStatus?.orderDetails);
                            }
    ...
    private async paymentHookOrderSuccessUpdate(methodName: string, methodId: number, orderId: string, order: any, statusId: number, orderValue: any, bcOrder: any, extras?: any) {
        ...
        if (bcOrder?.id && Utils.ParseUtil.matchStrings(orderId, bcOrder?.id, true)) {
            const dbOrderAmount = bcOrder?.grandTotal?.raw?.withTax || 0

            if (dbOrderAmount > 0) {
                let savePspInfo = Utils.PaymentUtil.getIsSavePSPInfo(methodId, order);

                const orderModel = {
                    id: (methodId === PaymentMethodTypeId.PAYPAL) ? extras?.paymentNo : Utils.PaymentUtil.getPaymentNo(methodId, order),
                    cardNo: null,
                    orderNo: (methodId === PaymentMethodTypeId.PAYPAL) ? extras?.orderNo : Utils.PaymentUtil.getOrderNo(methodId, order),
                    orderAmount: dbOrderAmount,
                    paidAmount: orderValue,
                    balanceAmount: '0.00',
                    isValid: true,
                    status: PaymentStatus.PAID,
                    authCode: Utils.PaymentUtil.getAuthCode(methodId, order),
                    issuerUrl: null,
                    paRequest: null,
                    pspSessionCookie: Utils.PaymentUtil.getSignature(methodId, order, extras?.hookData),
                    pspResponseCode: statusId,
                    pspResponseMessage: Utils.PaymentUtil.getPSPResponseMsg(methodId, order),
                    ...
                    paymentInfo1: Utils.PaymentUtil.getPSPInfo(methodId, order), // (pspInformation)
                    paymentInfo2: savePspInfo ? Utils.PaymentUtil.getPaymentIdentifier(methodId, order) : null, // (paymentIdentifier)
                    paymentInfo3: Utils.PaymentUtil.getPSPGatewayInfo(methodId, order), // (pspGatewayInfo)
                    paymentInfo4: Utils.PaymentUtil.getCardType(methodId, order), // (cardType)
                    paymentInfo5: Utils.PaymentUtil.getCardIssuer(methodId, order), // (cardIssuer)
                    paymentInfo6: Utils.PaymentUtil.getCardBrand(methodId, order), // (cardBrand)
                    cardType: null,
                    operatorId: null,
                    refStoreId: null,
                    tillNumber: null,
