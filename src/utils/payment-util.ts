// Package Imports
import { PayPalPayment } from "../domain/modules/payments/PayPalPayment"

// Other Imports
import { PaymentMethodType, PaymentMethodTypeId } from "../domain/enums"
import { Checkout, Defaults, PaymentTransactionStatus, Paypal, RegularExpression } from "../domain/constants/constants"
...
export default class PaymentUtil {
    ...
    /**
     * Creates a map of gateway names to their corresponding gateway IDs.
     * The map is case-insensitive, as the keys are converted to lowercase.
     * @returns {Map<string, number>} - A map of gateway names to their IDs.
     */
    static gatewayNameToIdMap() {
    ...
    /**
     * Creates a map of gateway IDs to their corresponding gateway names.
     * Filters out string keys to ensure only numeric IDs are mapped.
     * @returns {Map<number, string>} - A map of gateway IDs to their names.
     */
    static idToGatewayNameMap() {
    ...
    /*static getGatewayId(gatewayName: string): number {
     ...
     */
    static getGatewayId(gatewayName: string) {
        if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.PAYPAL, true)) {
            return PaymentMethodTypeId.PAYPAL
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.CHECKOUT, true)) {
            return PaymentMethodTypeId.CHECKOUT
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.KLARNA, true)) {
            return PaymentMethodTypeId.KLARNA
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.CLEAR_PAY, true)) {
            return PaymentMethodTypeId.CLEAR_PAY
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.MASTER_CARD, true)) {
            return PaymentMethodTypeId.MASTER_CARD
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.JUSPAY, true)) {
            return PaymentMethodTypeId.JUSPAY
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.STRIPE, true)) {
            return PaymentMethodTypeId.STRIPE
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.COD, true)) {
            return PaymentMethodTypeId.COD
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.ACCOUNT_CREDIT, true)) {
            return PaymentMethodTypeId.ACCOUNT_CREDIT
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.CHEQUE, true)) {
            return PaymentMethodTypeId.CHEQUE
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.CHECKOUT_APPLE_PAY, true)) {
            return PaymentMethodTypeId.CHECKOUT_APPLE_PAY
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.CHECKOUT_KLARNA, true)) {
            return PaymentMethodTypeId.CHECKOUT_KLARNA
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.ELAVON, true)) {
            return PaymentMethodTypeId.ELAVON
        } else if (Utils.ParseUtil.matchStrings(gatewayName, PaymentMethodType.OPAYO, true)) {
            return PaymentMethodTypeId.OPAYO
        }
        return -1
    }

    /*static getGatewayName(id: number): string | -1 {
     ...
     */
    static getGatewayName(id: number) {
     ...
     */
    static getPaymentTransactionStatus(methodId: number, data: any): string {
     ...
     */
    static async getPaymentTransactionOrderId(methodId: number, data: any): Promise<string> {
        if (methodId == PaymentMethodTypeId.CHECKOUT) {
            ...
            if (payPalOrderId) {
                ...
                if (paypalOrderDetails?.purchase_units?.length) {
                    ...
                    if (description) {
                        return PaymentUtil.parseOrderId(description)?.trim() || Defaults.String.Value;
                    }
                }
            }
        }
    ...
    /*static async getPaymentTransactionOrderNo(methodId: number, data: any): Promise<string> {
     ...
     */
    static getOrderNo(methodId: number, data: any, extras?: any): number {
        if (methodId == PaymentMethodTypeId.CHECKOUT) {
     ...
     */
    static getPaymentNo(methodId: number, data: any, extras?: any): string {
        if (methodId == PaymentMethodTypeId.CHECKOUT) {
     ...
     */
    static getAuthCode(methodId: number, data: any): string {
        if (methodId == PaymentMethodTypeId.CHECKOUT) {
            return data?.id;
        } else if (methodId == PaymentMethodTypeId.PAYPAL) {
            //return data?.id;
     ...
     *                   Returns a default string value if no relevant information is found.
     */
    static getSignature(methodId: number, data: any, hookData: any): string {
        if (methodId == PaymentMethodTypeId.CHECKOUT) {
     ...
     *                   Returns a default string value if no relevant information is found.
     */
    static getPSPResponseMsg(methodId: number, data: any): string {
        if (methodId == PaymentMethodTypeId.CHECKOUT) {
     ...
     */
    static getIsSavePSPInfo(methodId: number, data: any): boolean {
        ...
        if (methodId == PaymentMethodTypeId.CHECKOUT) {
            ...
            if (payGatewayId) {
                return Utils.ParseUtil.stringToBoolean(payGatewayId.split('_')[1]) ?? false;
            }
        }
        return false;
    }
     ...
     *                   Returns a default string value if no relevant information is found.
     */
    static getPSPInfo(methodId: number, data: any): string {
     ...
     */
    static getPaymentIdentifier(methodId: number, data: any): string {
     ...
     */
    static getPSPGatewayInfo(methodId: number, data: any): string {
     ...
     */
    static getCardType(methodId: number, data: any): string {

        if (methodId == PaymentMethodTypeId.CHECKOUT) {
            return data?.source?.card_type;
        } else if (methodId == PaymentMethodTypeId.JUSPAY) {
            return Utils.ParseUtil.matchStrings(data?.udf5, 'card', true)
                ? data?.card?.card_type
                : null;
        }
        return Defaults.String.Value;
     ...
     */
    static getCardIssuer(methodId: number, data: any): string {
     ...
     */
    static getCardBrand(methodId: number, data: any): string {
     ...
     */
    private static parseOrderId(input: string): string {
        const matches = Utils.ParseUtil.groupMatch(input?.trim(), new RegExp(RegularExpression.ORDER_BASKET_ID_MATCH));
