...
const SingletonFactory = (function () {
    ...
    const axiosInstance = axios.create({
        baseURL: SDKs.Paypal.Config.PayPalEnvironment.baseUrl,
    ...
    axiosInstance.interceptors.request.use(
        (config: any) => {
            ...
            const url = new URL('v1/oauth2/token', SDKs.Paypal.Config.PayPalEnvironment.getBaseUrl());
            const auth = Buffer.from(`${SDKs.Paypal.Config.PayPalEnvironment.getClientId()}:${SDKs.Paypal.Config.PayPalEnvironment.getAppSecret()}`).toString("base64");
            return axios({
                url: url.href,
...
const fetcher = async ({ url = '', method = 'post', data = {}, params = {}, headers = {}, cookies = {}, baseUrl = "", }: any) => {
    const computedUrl = new URL(url, baseUrl || SDKs.Paypal.Config.PayPalEnvironment.getBaseUrl());
    const config: any = {
        method: method,
