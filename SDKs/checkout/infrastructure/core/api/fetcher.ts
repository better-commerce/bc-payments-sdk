...
const SingletonFactory = (function () {
    ...
    const axiosInstance = axios.create({
        baseURL: SDKs.Checkout.Config.CheckoutEnvironment.baseUrl,
    ...
    function createAxiosResponseInterceptor() {
        const interceptor = axiosInstance.interceptors.response.use(
            ...
            (error: any) => {
                ...
                const extras = SDKs.Checkout.Config.CheckoutEnvironment.getExtras();
                ...
                const url = new URL('connect/token', SDKs.Checkout.Config.CheckoutEnvironment.getAuthUrl());
                const auth = Buffer.from(`${SDKs.Checkout.Config.CheckoutEnvironment.getAccessId()}:${SDKs.Checkout.Config.CheckoutEnvironment.getAccessSecret()}`).toString("base64");
                const requestConfig = {
                    url: url.href,
                    method: Domain.Enums.RequestMethod.POST,
                    data: `grant_type=client_credentials&client_id=${SDKs.Checkout.Config.CheckoutEnvironment.getAccessId()}&client_secret=${SDKs.Checkout.Config.CheckoutEnvironment.getAccessSecret()}&scope=gateway`,
                    headers: {
...
const fetcher = async ({ url = '', method = 'post', data = {}, params = {}, headers = {}, cookies = {}, baseUrl = "", }: any) => {
    const computedUrl = new URL(url, baseUrl || SDKs.Checkout.Config.CheckoutEnvironment.getBaseUrl());
    const config: any = {
        method: method,
